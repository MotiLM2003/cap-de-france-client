...
const ManageInventory = () => {
  ...
  const getHeaders = async () => {
    try {
      const { data } = await api.get('/inventroy/get-headers', {});
  ...
  const deleteHeader = async (header) => {
    const { data } = await api.post('/inventroy/remove-header', { header });
    console.log(data);
  };

  const createHeader = async (header) => {
    const { data } = await api.post('/inventroy/add-header', { header });
    console.log(data);
  };
  useEffect(() => {
    getHeaders();
  ...
  // on creating new inventory header
  const onCreateHeader = () => {
    const value = inputRef.current.value;
    if (!value) {
      toast.error('🐛 You must to provide an Header title.');
      return;
    }
    try {
      createHeader(value);
      ...
      <section className='manage-inventory__toolbar'>
        ...
        <AnimatePresence>
          {parseInt(currentHeader) > -1 && (
            ...
            >
              <i
                class='fas fa-trash-alt manage-inventory__trash button'
                onClick={onDelete}
              ></i>
            </motion.div>
          )}
